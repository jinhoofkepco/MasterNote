...
class ConstructionEmbeddedEditorTest {
    ...
    @Test fun `changed conflict token invalidates a confirmation already on screen`() {
        ...
        assertEquals("선생 노트·도형으로 학생 맞추기", choices.getButton(AlertDialog.BUTTON_POSITIVE).text.toString())
        assertEquals("학생 노트·도형으로 선생 맞추기", choices.getButton(AlertDialog.BUTTON_NEUTRAL).text.toString())
